...
const PatientLogin = () => {
  ...
  const patientEmails = [
    "sita@nepinsurance.com",
    "hari@nepinsurance.com",
    "ram@nepinsurance.com",
    "shyam@nepinsurance.com",
  ];
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (email && password) {
      if (rememberMe) {
        ...
        await Preferences.remove({ key: "patientLogin" });
      }

      await Preferences.set({ key: "patientLoggedIn", value: "true" });

      toast({
        title: "Login Successful",
        description: "Welcome to Patient Portal",
      });
      navigate("/staff-Home");
    } else {
      toast({
            ...
            <div className="flex items-center gap-3 p-3 border border-[#1ebac1] rounded-xl bg-white shadow-sm">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  if (value.length > 0) {
                    const matches = patientEmails.filter((item) =>
